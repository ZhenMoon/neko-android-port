...
async def websocket_endpoint(websocket: WebSocket, lanlan_name: str):
    ...
    try:
        ...
        while True:
            ...
            elif action == "screenshot_comment":
                # 悬浮窗检测到用户截屏 → 触发 YUI 主动搭话（真实 LLM 生成）。
                # 复用 greeting task gate：每角色同时只跑一个搭话类任务，避免
                # 与 reconnect / cat-return 问候并发抢占同一投递通道。
                logger.info("[%s] screenshot_comment: scheduling screenshot comment", lanlan_name)
                _schedule_greeting_task(
                    lanlan_name,
                    "screenshot",
                    lambda: session_manager[lanlan_name].trigger_screenshot_comment(),
                )
