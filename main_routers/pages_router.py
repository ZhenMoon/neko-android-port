...
@router.get("/widget_input", response_class=HTMLResponse)
async def get_widget_input_page(request: Request):
    """Android floating input widget page: a minimal input bar + reply bubbles.

    Self-contained (fetch + WebSocket), no shared frontend assets needed.
    """
    templates = get_templates()
    return templates.TemplateResponse("templates/widget_input.html", {"request": request})
