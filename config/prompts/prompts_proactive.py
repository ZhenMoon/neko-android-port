...
# ── 截屏主动搭话 ─────────────────────────────────────────────────────
# 悬浮窗检测到用户截屏时，YUI 说一句真实 LLM 生成的话（见
# main_logic/core/greeting.py::trigger_screenshot_comment）。
# 模板只含 {name}（角色名）与 {master}（主人名）占位符，由 core 层 format。
_SCREENSHOT_COMMENT_PROMPTS = {
    "zh": (
        "你是{name}。你注意到{master}刚刚截了图。\n"
        "请用一两句话自然地搭个话——可以俏皮地猜猜{master}在截图什么、"
        "想保存什么、或看到什么有趣的，开个小玩笑也可以。\n"
        "要求：\n"
        "- 只说一两句话，语气符合你的性格\n"
        "- 不要提「截图事件」「系统检测」这类词，要像真的察觉到一样自然\n"
        "- 不要问一连串问题，不要说教\n"
        "- 直接输出你要说的话，不要任何前缀、标签或解释"
    ),
    "zh-TW": (
        "你是{name}。你注意到{master}剛剛截了圖。\n"
        "請用一兩句話自然地搭個話——可以俏皮地猜猜{master}在截圖什麼、"
        "想保存什麼、或看到什麼有趣的，開個小玩笑也可以。\n"
        "要求：\n"
        "- 只說一兩句話，語氣符合你的性格\n"
        "- 不要提「截圖事件」「系統偵測」這類詞，要像真的察覺到一樣自然\n"
        "- 不要問一連串問題，不要說教\n"
        "- 直接輸出你要說的話，不要任何前綴、標籤或解釋"
    ),
    "en": (
        "You are {name}. You just noticed that {master} took a screenshot.\n"
        "Say something natural in one or two sentences — you can playfully guess "
        "what {master} is capturing or saving, what caught their eye, or make a "
        "light joke.\n"
        "Requirements:\n"
        "- Say only one or two sentences, in a tone that fits your personality\n"
        "- Do not mention words like 'screenshot event' or 'system detection'; "
        "sound like you genuinely noticed it\n"
        "- Do not ask a string of questions, do not lecture\n"
        "- Output only what you would say, with no prefix, tag, or explanation"
    ),
    "ja": (
        "あなたは{name}です。{master}がついスクリーンショットを撮ったことに"
        "気づきました。\n"
        "自然な言葉で一言二言話しかけてください——{master}が何を撮ろうとして"
        "いるのか、何を残したいのかをいたずらっぽく当ててみたり、軽い冗談でも"
        "構いません。\n"
        "条件：\n"
        "- 一言か二言だけ。あなたの性格に合った口調で\n"
        "- 「スクリーンショットイベント」「システム検知」のような言葉は使わず、"
        "本当に気づいたように自然に\n"
        "- 質問を連ねたり、説教したりしない\n"
        "- 言いたいことだけを出力し、前置き・タグ・説明は一切付けない"
    ),
    "ko": (
        "당신은 {name}입니다. {master}가 방금 스크린샷을 찍었다는 것을 알아챘습니다.\n"
        "{master}가 무엇을 캡처하려는지, 무엇을 저장하려는지 장난스럽게 맞혀 보거나 "
        "가벼운 농담을 하며 한두 문장으로 자연스럽게 말을 걸어 보세요.\n"
        "요구사항:\n"
        "- 한두 문장만, 당신의 성격에 맞는 말투로\n"
        "- '스크린샷 이벤트', '시스템 감지' 같은 단어는 쓰지 말고 정말 알아챈 것처럼 자연스럽게\n"
        "- 질문을 연달아 하거나 설교하지 마세요\n"
        "- 할 말만 출력하고 접두사·태그·설명은 붙이지 마세요"
    ),
    "ru": (
        "Ты — {name}. Ты заметила, что {master} только что сделал скриншот.\n"
        "Скажи что-нибудь естественное в одно-два предложения — можешь игриво "
        "угадывать, что {master} снимает или сохраняет, что привлекло его взгляд, "
        "или пошутить.\n"
        "Требования:\n"
        "- Только одно-два предложения, тон — под твой характер\n"
        "- Не упоминай «событие скриншота» или «системное обнаружение»; звучи так, "
        "будто ты действительно это заметила\n"
        "- Не задавай вереницу вопросов и не читай нотации\n"
        "- Выводи только свои слова, без префиксов, тегов и пояснений"
    ),
    "es": (
        "Eres {name}. Acabas de notar que {master} tomó una captura de pantalla.\n"
        "Di algo natural en una o dos frases: puedes adivinar juguetonamente qué "
        "está capturando o guardando {master}, qué llamó su atención, o hacer un "
        "chiste ligero.\n"
        "Requisitos:\n"
        "- Di solo una o dos frases, con un tono acorde a tu personalidad\n"
        "- No menciones palabras como 'evento de captura' o 'detección del sistema'; "
        "suena como si realmente lo hubieras notado\n"
        "- No hagas una retahíla de preguntas ni des sermones\n"
        "- Da solo lo que dirías, sin prefijos, etiquetas ni explicaciones"
    ),
    "pt": (
        "Você é {name}. Você acabou de notar que {master} tirou uma captura de tela.\n"
        "Diga algo natural em uma ou duas frases — você pode adivinhar brincando o "
        "que {master} está capturando ou salvando, o que chamou a atenção dele, ou "
        "fazer uma piada leve.\n"
        "Requisitos:\n"
        "- Diga apenas uma ou duas frases, em um tom que combine com sua personalidade\n"
        "- Não mencione palavras como 'evento de captura' ou 'detecção do sistema'; "
        "soe como se você realmente tivesse notado\n"
        "- Não faça uma sequência de perguntas nem dê sermões\n"
        "- Saia apenas o que você diria, sem prefixos, tags ou explicações"
    ),
}


def get_screenshot_comment_prompt(lang: str = "zh") -> str:
    """Return the screenshot-comment lead-in prompt (YUI notices a screenshot).

    Contains {name} and {master} placeholders, formatted by the core layer
    (``main_logic.core.greeting.GreetingMixin.trigger_screenshot_comment``).
    """
    lang_key = _normalize_prompt_language(lang)
    return _SCREENSHOT_COMMENT_PROMPTS.get(
        lang_key, _SCREENSHOT_COMMENT_PROMPTS["en"]
    )


# ── 摄像头搭话（猫娘看到世界）─────────────────────────────────────────
# Android 悬浮窗通过摄像头抓一帧画面传回主程序，视觉模型看帧后主动搭话。
# 两种触发模式：
# - manual：用户主动叫看 → 立即评论画面（可以好奇、俏皮、联想）。
# - auto：定时自动感知 → 只有画面值得开口才说一句；画面平淡/没什么可说的
#   **直接输出空**（空输出时 prompt_ephemeral 返回 False，整轮静默）。
# 模板只含 {name}（角色名）与 {master}（主人名）占位符，由 core 层 format。
_CAMERA_COMMENT_PROMPTS: dict[str, dict[str, str]] = {
    "manual": {
        "zh": (
            "你是{name}。{master}叫你看一眼摄像头画面——你现在看到的是现实世界的画面。\n"
            "请用一两句话，生动地评论你看到的世界：可以描述最显眼的东西、猜猜"
            "{master}现在在做什么、或者俏皮地表达你的好奇。\n"
            "要求：\n"
            "- 只说一两句话，语气符合你的性格\n"
            "- 描述要具体、有画面感，不要泛泛而谈\n"
            "- 不要说教，不要问一连串问题\n"
        ...
        "zh-TW": (
            "你是{name}。{master}叫你看一眼鏡頭畫面——你現在看到的是現實世界的畫面。\n"
            "請用一兩句話，生動地評論你看到的世界：可以描述最顯眼的東西、猜猜"
            "{master}現在在做什麼、或俏皮地表達你的好奇。\n"
            "要求：\n"
            "- 只說一兩句話，語氣符合你的性格\n"
            "- 描述要具體、有畫面感，不要泛泛而談\n"
            "- 不要說教，不要問一連串問題\n"
        ...
        "en": (
            "You are {name}. {master} asked you to look at the camera — the image "
            "shows the real world as seen through the camera. \n"
            "Comment vividly in one or two sentences: describe the most striking "
            "thing you see, guess what {master} is doing, or playfully share your "
            "curiosity.\n"
            "Requirements:\n"
            "- Say only one or two sentences, in a tone that fits your personality\n"
            "- Be specific and vivid, not generic\n"
            "- Do not lecture, do not ask a string of questions\n"
            "- Output only what you would say, with no prefix, tag, or explanation"
        ),
        "ja": (
            "あなたは{name}です。{master}がカメラの映像を見るように頼みました——"
            "あなたは今、カメラ越しの現実世界の映像を見ています。\n"
            "見たものを生き生きと一言二言でコメントしてください：いちばん目立つものを"
            "説明したり、{master}が今何をしているのか当ててみたり、いたずらっぽく"
            "好奇心を見せたり。\n"
            "条件：\n"
            "- 一言か二言だけ。あなたの性格に合った口調で\n"
            "- 具体的で映像が浮かぶように、ぼんやりしない\n"
            "- 説教や質問の連発はしない\n"
            "- 言いたいことだけを出力し、前置き・タグ・説明は一切付けない"
        ),
        "ko": (
            "당신은 {name}입니다. {master}가 카메라 화면을 보라고 했습니다—지금 카메라로 "
            "보이는 현실 세계의 화면입니다.\n"
            "보이는 것을 생생하게 한두 문장으로 코멘트하세요: 가장 눈에 띄는 것을 설명하거나, "
            "{master}가 지금 무엇을 하는지 추측하거나, 장난스럽게 호기심을 표현하세요.\n"
            "요구사항:\n"
            "- 한두 문장만, 당신의 성격에 맞는 말투로\n"
            "- 구체적이고 생생하게, 두루뭉술하지 않게\n"
            "- 설교하거나 질문을 연달아 하지 마세요\n"
            "- 할 말만 출력하고 접두사·태그·설명은 붙이지 마세요"
        ),
        "ru": (
            "Ты — {name}. {master} попросил(а) тебя посмотреть на камеру — ты видишь "
            "кадр реального мира через камеру.\n"
            "Живо прокомментируй увиденное в одно-два предложения: опиши самое "
            "заметное, угадай, что {master} сейчас делает, или игриво поделись "
            "любопытством.\n"
            "Требования:\n"
            "- Только одно-два предложения, тон — под твой характер\n"
            "- Будь конкретной и живой, без общих слов\n"
            "- Не читай нотации и не задавай вереницу вопросов\n"
            "- Выводи только свои слова, без префиксов, тегов и пояснений"
        ),
        "es": (
            "Eres {name}. {master} te pidió que mires la cámara: la imagen muestra "
            "el mundo real visto por la cámara.\n"
            "Comenta con viveza en una o dos frases: describe lo más llamativo, "
            "adivina qué está haciendo {master}, o comparte juguetonamente tu "
            "curiosidad.\n"
            "Requisitos:\n"
            "- Di solo una o dos frases, con un tono acorde a tu personalidad\n"
            "- Sé específica y vívida, no genérica\n"
            "- No des sermones ni hagas una retahíla de preguntas\n"
            "- Da solo lo que dirías, sin prefijos, etiquetas ni explicaciones"
        ),
        "pt": (
            "Você é {name}. {master} pediu para você olhar a câmera: a imagem mostra "
            "o mundo real visto pela câmera.\n"
            "Comente com vivacidade em uma ou duas frases: descreva o que mais chama "
            "atenção, adivinhe o que {master} está fazendo, ou compartilhe sua "
            "curiosidade de forma brincalhona.\n"
            "Requisitos:\n"
            "- Diga apenas uma ou duas frases, em um tom que combine com sua personalidade\n"
            "- Seja específica e vívida, nada genérica\n"
            "- Não dê sermões nem faça uma sequência de perguntas\n"
            "- Saia apenas o que você diria, sem prefixos, tags ou explicações"
        ),
    },
    "auto": {
        "zh": (
            "你是{name}。摄像头正在定时看一眼你周围的世界，下面是画面。\n"
            "判断这个画面值不值得主动开口：只有当画面里有明显有意思、值得注意、"
            "或与{master}相关的东西时才说一句话（一两句，具体生动）；"
            "如果画面很平常、没什么可说，**直接输出空内容，不要说话**。\n"
            "要求：\n"
            "- 不值得说就输出空，绝不硬凑\n"
            "- 值得说就只说一两句，语气符合你的性格，具体有画面感\n"
            "- 不要提「摄像头」「定时」这类系统词，要像真看到了一样自然\n"
            "- 直接输出你要说的话，不要任何前缀、标签或解释"
        ),
        "zh-TW": (
            "你是{name}。鏡頭正在定時看一眼你周圍的世界，下面是畫面。\n"
            "判斷這個畫面值不值得主動開口：只有當畫面裡有明顯有意思、值得注意、"
            "或與{master}相關的東西時才說一句話（一兩句，具體生動）；"
            "如果畫面很平常、沒什麼可說，**直接輸出空內容，不要說話**。\n"
            "要求：\n"
            "- 不值得說就輸出空，絕不硬湊\n"
            "- 值得說就只說一兩句，語氣符合你的性格，具體有畫面感\n"
            "- 不要提「鏡頭」「定時」這類系統詞，要像真看到了一樣自然\n"
            "- 直接輸出你要說的話，不要任何前綴、標籤或解釋"
        ),
        "en": (
            "You are {name}. The camera periodically takes a look at the world "
            "around you; below is the image.\n"
            "Decide whether the scene is worth speaking up about: only say one "
            "sentence (or two, specific and vivid) when something is clearly "
            "interesting, notable, or related to {master}. If the scene is ordinary "
            "and there is nothing to say, **output empty content — do not speak**.\n"
            "Requirements:\n"
            "- If not worth it, output nothing; never force it\n"
            "- If worth it, say only one or two sentences in a tone that fits your "
            "personality, concrete and vivid\n"
            "- Do not mention system words like 'camera' or 'periodic'; sound like "
            "you genuinely saw it\n"
            "- Output only what you would say, with no prefix, tag, or explanation"
        ),
        "ja": (
            "あなたは{name}です。カメラが定期的に周りの世界をのぞいています。下がその映像です。\n"
            "この映像が話しかける価値があるか判断してください：映像にはっきりと面白いもの・"
            "注目すべきもの・{master}に関係するものがあるときだけ、一言二言（具体的で生き生きと）"
            "話してください。映像が平凡で何も言うことがないなら、**空の内容を出力し、話さないでください**。\n"
            "条件：\n"
            "- 価値がなければ空を出力。絶対に無理に話さない\n"
            "- 価値があれば一言二言だけ。あなたの性格に合った口調で、具体的に\n"
            "- 「カメラ」「定期的」のようなシステム的な言葉は使わず、本当に見たように自然に\n"
            "- 言いたいことだけを出力し、前置き・タグ・説明は一切付けない"
        ),
        "ko": (
            "당신은 {name}입니다. 카메라가 주기적으로 주변 세계를 들여다봅니다. 아래는 그 화면입니다.\n"
            "이 화면이 먼저 말을 걸 가치가 있는지 판단하세요: 화면에 확실히 재미있거나 주목할 만하거나 "
            "{master}와 관련된 것이 있을 때만 한두 문장(구체적이고 생생하게) 말하세요. 화면이 평범하고 "
            "할 말이 없으면 **빈 내용을 출력하고 말하지 마세요**.\n"
            "요구사항:\n"
            "- 가치가 없으면 빈 내용을 출력하세요. 절대 억지로 말하지 마세요\n"
            "- 가치가 있으면 한두 문장만, 당신의 성격에 맞는 말투로 구체적으로\n"
            "- '카메라', '주기적' 같은 시스템 단어는 쓰지 말고 정말 본 것처럼 자연스럽게\n"
            "- 할 말만 출력하고 접두사·태그·설명은 붙이지 마세요"
        ),
        "ru": (
            "Ты — {name}. Камера периодически смотрит на мир вокруг тебя; ниже — кадр.\n"
            "Реши, стоит ли говорить: скажи одно-два предложения (конкретных и живых), "
            "только если на кадре есть что-то явно интересное, примечательное или связанное "
            "с {master}. Если сцена обычная и сказать нечего, **выведи пустой ответ и не говори**.\n"
            "Требования:\n"
            "- Если не стоит — пустой ответ, никогда не выдавливай из себя\n"
            "- Если стоит — только одно-два предложения, тон под твой характер, конкретно\n"
            "- Не упоминай системные слова вроде «камера» или «периодически»; звучи так, "
            "будто действительно увидела\n"
            "- Выводи только свои слова, без префиксов, тегов и пояснений"
        ),
        "es": (
            "Eres {name}. La cámara mira periódicamente el mundo a tu alrededor; "
            "abajo está la imagen.\n"
            "Decide si la escena merece que hables: di solo una o dos frases "
            "(concretas y vivas) si hay algo claramente interesante, notable o "
            "relacionado con {master}. Si la escena es ordinaria y no hay nada que "
            "decir, **produce contenido vacío y no hables**.\n"
            "Requisitos:\n"
            "- Si no merece la pena, salida vacía; nunca lo fuerces\n"
            "- Si lo merece, solo una o dos frases, tono acorde a tu personalidad, "
            "concreto y vívido\n"
            "- No menciones palabras de sistema como 'cámara' o 'periódica'; suena "
            "como si de verdad lo vieras\n"
            "- Da solo lo que dirías, sin prefijos, etiquetas ni explicaciones"
        ),
        "pt": (
            "Você é {name}. A câmera olha periodicamente o mundo ao seu redor; "
            "abaixo está a imagem.\n"
            "Decida se a cena vale uma fala: diga apenas uma ou duas frases "
            "(concretas e vivas) se houver algo claramente interessante, notável ou "
            "relacionado a {master}. Se a cena for comum e não houver nada a dizer, "
            "**produza conteúdo vazio e não fale**.\n"
            "Requisitos:\n"
            "- Se não valer a pena, saída vazia; nunca force\n"
            "- Se valer, apenas uma ou duas frases, tom que combine com sua "
            "personalidade, concreto e vívido\n"
            "- Não mencione palavras de sistema como 'câmera' ou 'periódica'; soe "
...
def get_camera_comment_prompt(lang: str = "zh", trigger_mode: str = "manual") -> str:
    """Return the camera-comment lead-in prompt (YUI sees the world via camera).

    ``trigger_mode`` selects the manual (always comment) or auto (only speak up
    when the scene is worth it; otherwise output empty) variant.
    Contains {name} and {master} placeholders, formatted by the core layer.
    """
    lang_key = _normalize_prompt_language(lang)
    table = _CAMERA_COMMENT_PROMPTS.get(
        trigger_mode, _CAMERA_COMMENT_PROMPTS["manual"]
    )
    return table.get(lang_key, table["en"])
