...
function createSettingsPopupContent(manager, prefix, popup) {
    ...
    // 5. 添加导航菜单（安卓 WebView 也视为桌面端，保留 API密钥/记忆浏览入口）
    {
...
const AvatarPopupMixin = {
    apply: function (ManagerProto, prefix, options = {}) {
        ...
        ManagerProto._createSettingsMenuItems = function (popup) {
            ...
            // 安卓 App 专属：宠物悬浮窗（SYSTEM_ALERT_WINDOW 权限由原生层引导）。
            if (window.nekoAndroid && typeof window.nekoAndroid.openFloatingWindow === 'function') {
                settingsItems.push({
                    id: 'floating-window',
                    label: '宠物悬浮窗',
                    icon: '/static/icons/memory_icon.png',
                    action: 'floating'
                });
            }
            ...
            settingsItems.forEach(item => {
                ...
                if (item.action === 'floating' && window.nekoAndroid) {
                    menuItem.addEventListener('click', (e) => {
                        e.stopPropagation();
                        try {
                            window.nekoAndroid.openFloatingWindow();
                        } catch (err) {
                            console.warn('[AvatarUI] openFloatingWindow failed:', err);
                        }
                    });
                }
            });
