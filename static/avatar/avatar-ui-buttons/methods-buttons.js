(function setupSocialUnlockState() {
    ...
    function readUnlockEvidence(todayDate) {
        // 安卓 App 视作老用户直接解锁喵宇宙，跳过新用户 3 天冷却（安卓没有
        // 七天教程数据，永远按新用户算）。?pet=1 是主页入口；二级页用
        // location.href='/' 返回会丢参数，用 __LANLAN_IS_ANDROID_PET__ 兜底。
        if (window.__LANLAN_IS_ANDROID_PET__ === true || /[?&]pet=1/.test(location.search || '')) {
            return { firstSeenDate: getLocalDateKey(todayDate), existingUser: true };
        }
