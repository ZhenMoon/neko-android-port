(function () {
    ...
    // ---- 记忆备份导出 / 导入（跨端迁移） ----
    let memoryBackupBusy = false;

    function setMemoryBackupStatus(message, state, dismissAfterMs) {
        const status = document.getElementById('memory-export-logs-status');
        if (!status) return;
        clearMemoryExportLogsStatusTimer();
        status.textContent = String(message || '');
        ...
        if (dismissAfterMs > 0) {
            memoryExportLogsStatusTimer = window.setTimeout(function () {
                memoryExportLogsStatusTimer = 0;
                setMemoryBackupStatus('', '');
            }, dismissAfterMs);
        }
    }

    function handleMemoryBackupExport() {
        if (memoryBackupBusy) return;
        memoryBackupBusy = true;
        setMemoryBackupStatus('正在打包记忆…', 'pending');
        const anchor = document.createElement('a');
        anchor.href = '/api/memory/export?t=' + Date.now();
        anchor.download = 'neko-memory-backup.tar.gz';
        document.body.appendChild(anchor);
        anchor.click();
        anchor.remove();
        setMemoryBackupStatus('已开始导出，请检查下载目录', 'success', MEMORY_EXPORT_LOGS_SUCCESS_MS);
        memoryBackupBusy = false;
    }

    function handleMemoryBackupImportClick() {
        const input = document.getElementById('memory-backup-import-file');
        if (!input || memoryBackupBusy) return;
        input.value = '';
        input.click();
    }

    async function handleMemoryBackupImportFile(event) {
        const input = event.target;
        const file = input && input.files && input.files[0];
        if (!file || memoryBackupBusy) return;
        memoryBackupBusy = true;
        setMemoryBackupStatus('正在导入记忆…', 'pending');
        try {
            const form = new FormData();
            form.append('file', file);
            const response = await fetch('/api/memory/import', { method: 'POST', body: form });
            ...
            if (response.ok && result && result.ok) {
                ...
                if (names.length) {
                    ...
                    broadcastExternalMemoryEdited(names[0]);
                } else {
                    setMemoryBackupStatus(
                        '备份中没有角色记忆',
                        'success',
                ...
                setMemoryBackupStatus(
                    '导入失败：' + String((result && result.detail) || '请检查文件是否有效'),
                    'error',
                    MEMORY_EXPORT_LOGS_ERROR_MS
                );
            }
        } catch (_) {
            setMemoryBackupStatus('导入失败，请重试', 'error', MEMORY_EXPORT_LOGS_ERROR_MS);
        } finally {
            memoryBackupBusy = false;
            input.value = '';
        }
    ...
    function initMemoryBackupTransfer() {
        const exportBtn = document.getElementById('memory-backup-export-trigger');
        const importBtn = document.getElementById('memory-backup-import-trigger');
        const input = document.getElementById('memory-backup-import-file');
        if (exportBtn) exportBtn.addEventListener('click', handleMemoryBackupExport);
        if (importBtn) importBtn.addEventListener('click', handleMemoryBackupImportClick);
        if (input) input.addEventListener('change', handleMemoryBackupImportFile);
    }
