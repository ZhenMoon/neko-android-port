...
# 截屏搭话的最小间隔（秒）：截图连拍 / 滚动截屏时防止反复刷搭话。
_SCREENSHOT_COMMENT_THROTTLE_SECONDS = 45.0
# 摄像头搭话节流：手动 15s（防止快速连点），定时自动 90s（防止高频抓帧
# + 每次 LLM 判定都花钱）。
_CAMERA_COMMENT_MANUAL_THROTTLE_SECONDS = 15.0
_CAMERA_COMMENT_AUTO_THROTTLE_SECONDS = 90.0
...
class GreetingMixin:
    ...
    async def trigger_screenshot_comment(self) -> None:
        """When the user takes a screenshot (floating pet widget), YUI notices it
        and says a short real-LLM comment.

        Lighter than ``trigger_greeting``: no memory-gap query, no startup
        history bookkeeping. The guards mirror ``trigger_cat_greeting`` (goodbye
        silent / voice session / takeover / atomic SM claim), plus a 45-second
        throttle so a screenshot burst cannot spam the user.
        """
        if self.is_goodbye_silent():
            logger.info("[%s] trigger_screenshot_comment: goodbye silent, skipping", self.lanlan_name)
            return
        if self._is_voice_session_active_or_starting():
            logger.info("[%s] trigger_screenshot_comment: voice session active/starting, skipping", self.lanlan_name)
            return
        if self._takeover_active:
            logger.info("[%s] trigger_screenshot_comment: session takeover active, skipping", self.lanlan_name)
            return

        # 节流：45 秒内最多搭一次话，截图连拍 / 滚动截屏不刷屏。
        last_at = getattr(self, "_last_screenshot_comment_at", 0.0)
        now = time.time()
        if now - last_at < _SCREENSHOT_COMMENT_THROTTLE_SECONDS:
            logger.debug(
                "[%s] trigger_screenshot_comment: throttled (%.0fs ago)",
                self.lanlan_name, now - last_at,
            )
            return

        from config.prompts.prompts_proactive import (
            get_screenshot_comment_prompt,
            normalize_proactive_prompt_locale,
        )
        _lang = normalize_proactive_prompt_locale(
            getattr(self, 'user_language', '') or get_global_language_full()
        )
        template = get_screenshot_comment_prompt(_lang)
        instruction = template.format(name=self.lanlan_name, master=self.master_name)
        logger.info(
            "[%s] trigger_screenshot_comment: delivering (instruction len=%d)",
            self.lanlan_name, len(instruction),
        )

        # 投递通道：已有空闲 text session 则直接用，否则主动拉起（与
        # trigger_cat_greeting 对偶）。
        if isinstance(self.session, OmniOfflineClient) and not getattr(self.session, "_is_responding", False):
            pass
        else:
            if self._is_voice_session_active_or_starting():
                logger.info("[%s] trigger_screenshot_comment: voice session appeared before text session auto-start, skipping", self.lanlan_name)
                return
            if not self._has_connected_websocket():
                logger.warning("[%s] trigger_screenshot_comment: no connected websocket, aborting", self.lanlan_name)
                return
            try:
                logger.info("[%s] trigger_screenshot_comment: auto-starting text session", self.lanlan_name)
                await self.start_session(self.websocket, new=False, input_mode='text')
            except Exception as e:
                logger.warning("[%s] trigger_screenshot_comment: auto start_session failed: %s", self.lanlan_name, e)
                return

        if not isinstance(self.session, OmniOfflineClient):
            logger.warning("[%s] trigger_screenshot_comment: session is not text mode after start, aborting", self.lanlan_name)
            return

        # ── 投递前最终检查：构建 instruction 期间语音可能已接管 ──
        if self._is_voice_session_active_or_starting():
            logger.info("[%s] trigger_screenshot_comment: voice session took over before delivery, skipping", self.lanlan_name)
            return

        # 原子 SM claim：与 trigger_greeting / trigger_agent_callbacks / proactive_chat 互斥
        if not await self.state.try_start_proactive(session=self.session):
            logger.info(
                "[%s] trigger_screenshot_comment: SM denied claim (phase=%s), skipping",
                self.lanlan_name, self.state.phase.value,
            )
            return
        ...
        try:
            async with self._proactive_write_lock:
                if self._is_voice_session_active_or_starting():
                    logger.info("[%s] trigger_screenshot_comment: voice session took over while waiting for write lock, skipping", self.lanlan_name)
                    return
                async with self.lock:
                    if self.state.is_proactive_preempted():
                        logger.info("[%s] trigger_screenshot_comment: preempted before sid claim, skipping", self.lanlan_name)
                        return
                    self.current_speech_id = str(uuid4())
                    self._tts_done_queued_for_turn = False
                    self._tts_done_pending_until_ready = False
                ...
                try:
                    ...
                    if not isinstance(session_ref, OmniOfflineClient):
                        logger.info(
                            "[%s] trigger_screenshot_comment: session swapped/nullified "
                            "before prompt_ephemeral (now=%s), skipping",
                            self.lanlan_name, type(session_ref).__name__,
                        )
                        return
                    # 只有真正进入投递才记时间戳，节流不因被 guard 拦住的
                    # 请求而累积。
                    self._last_screenshot_comment_at = now
                    delivered = await session_ref.prompt_ephemeral(instruction)
                finally:
                    _proactive_expected_sid.reset(_sid_token)
                logger.info("[%s] trigger_screenshot_comment: delivered=%s", self.lanlan_name, delivered)
        finally:
            await self.state.fire(SessionEvent.PROACTIVE_DONE)

    async def trigger_camera_comment(self, image_b64: str, trigger_mode: str = "manual") -> None:
        """YUI sees the world through the Android camera and says a real-LLM comment.

        The image frame (JPEG base64) is injected into ``prompt_ephemeral`` as
        visual context (the offline client switches to the vision model when
        ``images`` is present). Two modes:

        - ``manual``: user asked her to look → always comment, short throttle so
          rapid taps cannot spam.
        - ``auto``: periodic camera glance → the prompt tells her to output empty
          content when the scene is not worth speaking up about; an empty turn
          makes ``prompt_ephemeral`` return False and stay silent.

        Guards mirror ``trigger_screenshot_comment`` (goodbye silent / voice /
        takeover / atomic SM claim).
        """
        if not image_b64:
            return
        if self.is_goodbye_silent():
            logger.info("[%s] trigger_camera_comment: goodbye silent, skipping", self.lanlan_name)
            return
        if self._is_voice_session_active_or_starting():
            logger.info("[%s] trigger_camera_comment: voice session active/starting, skipping", self.lanlan_name)
            return
        if self._takeover_active:
            logger.info("[%s] trigger_camera_comment: session takeover active, skipping", self.lanlan_name)
            return

        # 节流：手动 15s 内不重复；自动（定时感知）90s 内不重复，避免
        # 高频抓帧 + LLM 每次判定都花钱。
        throttle = (
            _CAMERA_COMMENT_MANUAL_THROTTLE_SECONDS
            if trigger_mode == "manual"
            else _CAMERA_COMMENT_AUTO_THROTTLE_SECONDS
        )
        last_at = getattr(self, "_last_camera_comment_at", 0.0)
        now = time.time()
        if now - last_at < throttle:
            logger.debug(
                "[%s] trigger_camera_comment: throttled (%.0fs ago)",
                self.lanlan_name, now - last_at,
            )
            return

        from config.prompts.prompts_proactive import (
            get_camera_comment_prompt,
            normalize_proactive_prompt_locale,
        )
        _lang = normalize_proactive_prompt_locale(
            getattr(self, 'user_language', '') or get_global_language_full()
        )
        template = get_camera_comment_prompt(_lang, trigger_mode=trigger_mode)
        instruction = template.format(name=self.lanlan_name, master=self.master_name)
        logger.info(
            "[%s] trigger_camera_comment: delivering mode=%s (instruction len=%d)",
            self.lanlan_name, trigger_mode, len(instruction),
        )

        # 投递通道：已有空闲 text session 则直接用，否则主动拉起（与
        # trigger_screenshot_comment 对偶）。
        if isinstance(self.session, OmniOfflineClient) and not getattr(self.session, "_is_responding", False):
            pass
        else:
            if self._is_voice_session_active_or_starting():
                logger.info("[%s] trigger_camera_comment: voice session appeared before text session auto-start, skipping", self.lanlan_name)
                return
            if not self._has_connected_websocket():
                logger.warning("[%s] trigger_camera_comment: no connected websocket, aborting", self.lanlan_name)
                return
            try:
                logger.info("[%s] trigger_camera_comment: auto-starting text session", self.lanlan_name)
                await self.start_session(self.websocket, new=False, input_mode='text')
            except Exception as e:
                logger.warning("[%s] trigger_camera_comment: auto start_session failed: %s", self.lanlan_name, e)
                return

        if not isinstance(self.session, OmniOfflineClient):
            logger.warning("[%s] trigger_camera_comment: session is not text mode after start, aborting", self.lanlan_name)
            return

        # ── 投递前最终检查：构建 instruction 期间语音可能已接管 ──
        if self._is_voice_session_active_or_starting():
            logger.info("[%s] trigger_camera_comment: voice session took over before delivery, skipping", self.lanlan_name)
            return

        # 原子 SM claim：与 trigger_greeting / trigger_agent_callbacks / proactive_chat 互斥
        if not await self.state.try_start_proactive(session=self.session):
            logger.info(
                "[%s] trigger_camera_comment: SM denied claim (phase=%s), skipping",
                self.lanlan_name, self.state.phase.value,
            )
            return
        ...
        try:
            async with self._proactive_write_lock:
                if self._is_voice_session_active_or_starting():
                    logger.info("[%s] trigger_camera_comment: voice session took over while waiting for write lock, skipping", self.lanlan_name)
                    return
                async with self.lock:
                    if self.state.is_proactive_preempted():
                        logger.info("[%s] trigger_camera_comment: preempted before sid claim, skipping", self.lanlan_name)
                        return
                    self.current_speech_id = str(uuid4())
                    self._tts_done_queued_for_turn = False
                    self._tts_done_pending_until_ready = False
                ...
                try:
                    ...
                    if not isinstance(session_ref, OmniOfflineClient):
                        logger.info(
                            "[%s] trigger_camera_comment: session swapped/nullified "
                            "before prompt_ephemeral (now=%s), skipping",
                            self.lanlan_name, type(session_ref).__name__,
                        )
                        return
                    # 只有真正进入投递才记时间戳，节流不因被 guard 拦住的请求而累积。
                    self._last_camera_comment_at = now
                    delivered = await session_ref.prompt_ephemeral(
                        instruction,
                        images=[image_b64],
                    )
                    ...
                    _proactive_expected_sid.reset(_sid_token)
                logger.info("[%s] trigger_camera_comment: delivered=%s", self.lanlan_name, delivered)
        finally:
            await self.state.fire(SessionEvent.PROACTIVE_DONE)
